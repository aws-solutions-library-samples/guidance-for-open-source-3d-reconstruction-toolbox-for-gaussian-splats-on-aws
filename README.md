...
- create an infrastructure required to create a Gaussian splat from a video or set of images
- create a mechanism to run the code and perform 3D reconstruction
...
<div align="center">
<img src="assets/images/gs-deploy-arch-new.png" width=70%> 
<br/>
<i>Figure 1: 3D Reconstruction Toolbox for Gaussian Splats on AWS Deployment Architecture</i>
</div>

**Deployment Architecture Steps:**
1. Administrator user deploys guidance to AWS account and Region using AWS Cloud Development Kit (CDK) or Terraform.
...
3. Once the Base AWS Cloud Formation stack has been deployed, the Post Deploy AWS Cloud Formation stack should be deployed. That stack will build a Docker container either locally or using **AWS CodeBuild**, push it to the Amazon ECR registry, and build and push the pre-processing models used during training (such as for background removal) into Amazon S3 bucket using AWS Lambda.


### Workflow Architecture

<div align="center">
<img src="assets/images/gs-workflow-arch-new.png" width=70%> 
<br/>
<i>Figure 2: 3D Reconstruction Toolbox for Gaussian Splats on AWS Reference Architecture</i>
</div>
...
| Training         | Images, Point Cloud, & Poses to Gaussian Splat               | [NerfStudio](https://github.com/nerfstudio-project/nerfstudio) [(Apache-2.0)](https://github.com/nerfstudio-project/nerfstudio/tree/main?tab=Apache-2.0-1-ov-file#readme), [gsplat](https://github.com/nerfstudio-project/gsplat) [(Apache-2.0)](https://github.com/nerfstudio-project/gsplat?tab=Apache-2.0-1-ov-file#readme), [splatfacto-w](https://github.com/KevinXu02/splatfacto-w) [(Apache-2.0)](https://github.com/KevinXu02/splatfacto-w?tab=Apache-2.0-1-ov-file#readme), [DN-Splatter](https://github.com/maturk/dn-splatter) [(Apache-2.0)](https://github.com/maturk/dn-splatter?tab=Apache-2.0-1-ov-file), [AGS-Mesh](https://github.com/maturk/dn-splatter) [(Apache-2.0)](https://github.com/maturk/dn-splatter?tab=Apache-2.0-1-ov-file), [3DGRUT](https://github.com/nv-tlabs/3dgrut) [(Apache-2.0)](https://github.com/nv-tlabs/3dgrut#Apache-2.0-1-ov-file) | Yes | Supports splatfacto, splatfacto-big, splatfacto-mcmc, splatfacto-w-light, dn-splatter, ags-mesh, 3dgrt, 3dgut, nerfacto. Gsplat is used for multi-gpu and depth loss training exclusively              |
...
| Post Processing  | Export Gaussian Splat and Metadata                           | [Splat-Transform](https://github.com/playcanvas/splat-transform) [(MIT)](https://github.com/playcanvas/splat-transform#MIT-1-ov-file), [spz](https://github.com/nianticlabs/spz/tree/main) [(MIT)](https://github.com/nianticlabs/spz/tree/main#MIT-1-ov-file) | Yes | Supports output .ply, .spz, .sog, .usdz (beta), .mp4(splat render), .jpg(thumbnail), metrics(psnr, lpips, ssim)                                                                               |
| Post Processing  | Export Mesh (only DN-Splatter or AGS-Mesh)                           | [DN-Splatter](https://github.com/maturk/dn-splatter) [(Apache-2.0)](https://github.com/maturk/dn-splatter?tab=Apache-2.0-1-ov-file) | Yes | Supports output colored .glb                                                                               |
...
| Post Processing  | Generate collision voxel octree and mesh from splat          | [Splat-Transform](https://github.com/playcanvas/splat-transform) [(MIT)](https://github.com/playcanvas/splat-transform#MIT-1-ov-file) | Yes | Outputs .voxel.json/.voxel.bin (SVO) and .collision.glb; supports indoor, outdoor, object scene types |
| Post Processing  | Generate streamed multi-LOD SOG bundle from splat            | [Splat-Transform](https://github.com/playcanvas/splat-transform) [(MIT)](https://github.com/playcanvas/splat-transform#MIT-1-ov-file) | No  | Outputs lod-meta.json with 4 decimation levels (100%, 50%, 25%, 12.5%) for progressive streaming |
| Post Processing  | Clean Splat (remove floaters)            | [Splat-Transform](https://github.com/playcanvas/splat-transform) [(MIT)](https://github.com/playcanvas/splat-transform#MIT-1-ov-file) | Yes  | |
| Post Processing  | Crop all generated objects                                   | | No | Supports environments or rigid_objects to reduce noise.             |
...
### Sample Gaussian Splat Outputs
A collection of compressed splats can be found in the repo at [source/Gradio/favorites](source/Gradio/favorites) and include both `.spz` and `.sog` example outputs. These outputs can be viewed/edited inside of [SuperSplat](https://superspl.at/editor) and are raw results from the pipeline that demonstrate:
- Segmentation (background removal)
- Cropping (auto rigid body crop)
- Exterior and interior spaces
- Spherical (panorama image input)
- Typical SotA quality demonstrated with splats
...
_You are responsible for the cost of the AWS services used while running this Guidance. As of June 2026, the cost for running this Guidance with the default settings in the default AWS Region (US East 1(N. Virginia)) is approximately $73.33 per month for processing 100 requests._
