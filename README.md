...
| Error   | False Positive  | CodeQL       | Path injection — generate_splat_gradio.py:1095,1113,1791 | Uncontrolled data used in path expression | Lines 1095 & 1113: config_name is sanitized with regex, truncated to 64 chars, and validated with `os.path.realpath()` check before file operations. Line 1791: filename is validated with `os.path.basename()` and checked for '..' to prevent traversal. CodeQL does not recognize these validation patterns. |
| Error   | False Positive  | Grype        | GHSA-p436-gjf2-799p — github.com/docker/cli v28.1.1 | Docker CLI plugins uncontrolled search path element leads to local privilege escalation on Windows | This guidance deploys exclusively on Linux-based AWS infrastructure. The vulnerability requires local access to a Windows host and does not apply to this deployment. |
| Error   | False Positive  | Grype        | GHSA-9h8m-3fm2-qjrq — go.opentelemetry.io/otel/sdk v1.35.0 | OpenTelemetry Go SDK vulnerable to arbitrary code execution via PATH hijacking | This is a transitive build-time dependency of the CDK toolchain, not a runtime dependency of the deployed guidance. The vulnerability requires a malicious executable on the PATH of the build host, which is outside the threat model of this guidance. |
| Error   | False Positive  | Grype        | GHSA-pwhc-rpq9-4c8w — github.com/containerd/containerd/v2 v2.0.5 | containerd affected by local privilege escalation via wide permissions on CRI directory | This is a transitive build-time dependency of the CDK/Docker toolchain, not a runtime component of the deployed guidance. The CRI directory permissions issue requires local access to the build host and does not affect the deployed AWS infrastructure. |
| Error   | False Positive  | Codeguru/Semgrep | NoSQL Injection — clean_point_cloud.py:81, clean_ply.py:44 | Untrusted input used in NoSQL database operation without sanitization | Neither file contains any DynamoDB or NoSQL operations. The scanner incorrectly flagged `tree.query()` (a scipy KD-tree spatial query) and `content.find()` (a bytes search) as NoSQL database calls. These are pure in-memory data processing operations with no database interaction. |
| Error   | False Positive  | Codeguru/Semgrep | Path traversal — container pipeline files (utils.py, colmap.py, remove_background.py, and others) | User-controlled input in file paths across multiple pipeline modules | All file paths in the container pipeline originate from the job configuration, which is validated via `validate_input_media()` and `os.path.realpath()` checks in `main.py` before being passed to any pipeline module. The scanner does not trace validation through the call chain from `main.py` into the individual pipeline modules. |
