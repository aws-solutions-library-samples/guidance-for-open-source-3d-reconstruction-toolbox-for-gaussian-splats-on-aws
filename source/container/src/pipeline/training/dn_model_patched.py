...
class DNSplatterModel(SplatfactoModel):
    ...
    def get_outputs(
        ...
        if self.config.predict_normals:
            ...
            normals_render, _, _, _, _, _, _ = rasterization_2dgs(
                means=means_crop,
                quats=quats_crop / quats_crop.norm(dim=-1, keepdim=True),
                scales=torch.exp(scales_crop).float(),
                opacities=torch.sigmoid(opacities_crop).squeeze(-1),
                colors=normals.unsqueeze(0),  # [1, N, 3] batch dim for cameras
                viewmats=viewmat,  # [1, 4, 4]
                Ks=K,  # [1, 3, 3]
                width=W,
                height=H,
            ...
            # normals_render shape: [1, H, W, 3] -> squeeze batch dim, normalize, remap to [0,1]
            normals_im = normals_render.squeeze(0)
            normals_im = normals_im / (normals_im.norm(dim=-1, keepdim=True) + 1e-8)
    ...
    def get_image_metrics_and_images(
        ...
        predicted_depth = outputs["depth"]
        # Squeeze batch dim if present: [1,H,W,1] -> [H,W,1]
        if predicted_depth.dim() == 4:
