...
"""
Demo script to export MapAnything outputs in COLMAP format with DA3 support.

This script runs MapAnything or DA3 inference on images and exports the results
to COLMAP format with scene-adaptive voxel downsampling.

Usage:
    python demo_colmap.py --images_dir=/path/to/images/ --output_dir=/path/to/output/ --model=da3
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="MapAnything COLMAP Export Demo with DA3 support",
...
def main():
    ...
    # Print configuration
    print("=" * 60)
    print("MapAnything COLMAP Export with DA3")
    print("=" * 60)
    ...
    if args.model == "da3":
        model = init_model("da3", device=device)
    elif args.apache:
    ...
    print(f"Running {args.model} inference...")
    with torch.no_grad():
        outputs = model.infer(
            views,
